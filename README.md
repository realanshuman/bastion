...
**An agentic security tool for developers. It stops hidden malware in your JavaScript projects before it runs. When something gets in, it investigates, contains it and tells you exactly what's left to do.**
...
<div align="center"><img src="assets/window-incident.png" width="820" alt="What needs you, with proof and a one-click fix"/></div>
...
The same questions work in a terminal. Nothing leaves your Mac: Bastion matches the question to what it already
knows and answers in its own words.

```bash
bastion ask "is my-app safe?"        # checks that repo in a couple of seconds
bastion ask "what needs me?"         # what's left, with the command that fixes each one
bastion ask "what does dormant mean?"
bastion ask "turn on protection"     # switches on everything that's off (turning things off stays with you)
```
